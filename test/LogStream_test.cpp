...
MTEST(testLogStreamBooleans)
{
  rawmd::LogStream os;
  const rawmd::LogStream::Buffer& buf = os.buffer();
...
MTEST(testLogStreamIntegerLimits)
{
  rawmd::LogStream os;
  const rawmd::LogStream::Buffer& buf = os.buffer();
  os << -2147483647;
...
MTEST(testLogStreamFmts)
{
  ...
  os << rawmd::Fmt("%4.2f", 1.2);
  EXPECT_EQ(buf.toString(), string("1.20"));
  os.resetBuffer();

  os << rawmd::Fmt("%4.2f", 1.2) << rawmd::Fmt("%4d", 43);
  EXPECT_EQ(buf.toString(), string("1.20  43"));
  os.resetBuffer();
}

MTEST(testLogStreamLong)
{
  rawmd::LogStream os;
  const rawmd::LogStream::Buffer& buf = os.buffer();
  for (int i = 0; i < 399; ++i)
